      ...
      <div className="max-w-4xl mx-auto p-4 relative">
        ...
        <div className="bg-blue-50 border border-blue-400 rounded-2xl shadow-md p-6 mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-800">
            ...
            <p><span className="font-medium">Tipo:</span> {user.role == 'entrepreneur' ? 'empreendedor' : 'cliente'}</p>
            <p><span className="font-medium">Criado em:</span> {new Date(user.createdAt).toLocaleDateString('pt-BR')}</p>

            {user.slug && (
              <p><span className="font-medium">Minha tag:</span> @{user.slug}</p>
            )}
          </div>
        </div>
          ...
          {user.appointmentsAsClient?.length ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {user.appointmentsAsClient.map((appt) => (
                ...
                >
                  <div>
                    <p className="text-sm text-gray-800 mb-1"><strong>Empresário:</strong> {appt.entrepreneur.businessName}</p>
                    <p className="text-sm text-gray-800 mb-1">
                      <strong>Data:</strong> {" "}
                      {appt.date.replace(/T.*/, '').split('-').reverse().join('/')}
                    </p>
        ...
        {/* AGENDAMENTOS COMO EMPRESÁRIO */}
        {user.role === "entrepreneur" && (
          <div className="mb-12">
            <h2 className="text-xl font-semibold mb-4 text-gray-700">Agendados comigo</h2>
            {user.appointmentsAsEntrepreneur?.length ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {user.appointmentsAsEntrepreneur.map((appt) => (
                  <div
                    key={appt.id}
                    className="bg-white border border-green-400 rounded-xl shadow-sm p-4 h-52 flex flex-col justify-between transition-colors duration-300 hover:bg-green-100"
                  >
                    <div>
                      <p className="text-sm text-gray-800 mb-1"><strong>Cliente:</strong> {appt.client?.name ?? "Desconhecido"}</p>
                      <p className="text-sm text-gray-800 mb-1">
                        <strong>Data:</strong> {" "}
                        {appt.date.replace(/T.*/, '').split('-').reverse().join('/')}
                      </p>
              ...
              <p className="text-gray-500">Nenhum cliente agendou com você ainda.</p>
            )}
          </div>
        )}
      </div>
